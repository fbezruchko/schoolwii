...
#define RX_COND
  ...
  while (cc-- RX_COND) {
    ...
    // Check for stalled GPS, if no frames seen for 3.2sec then consider it LOST
    if ((timeMax - GPS_last_frame_seen) > 3200000) {
      //No update since 3200ms clear fix...
      f.GPS_FIX = 0;
      GPS_numSat = 0;
    }
...
void evaluateCommand(uint8_t c) {
  ...
  switch (c) {
    ...
    case MSP_IDENT:
      ...
      id.cap   = 0 | 0 << 2 | 0 << 3 | NAVCAP << 4 | 0 << 5 | ((uint32_t)NAVI_VERSION << 28); //Navi version is stored in the upper four bits;
